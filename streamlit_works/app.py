...
input_file = st.sidebar.file_uploader("入力ファイル", type=["xlsx"])

tab1, tab2, tab3, tab4 = st.tabs(["カレンダー情報", "スタッフ情報", "シフト表作成", "確定シフト"])
...
with tab4:
    if input_file is None:
        st.write("入力ファイルをアップロードしてください")
    else:
        st.markdown("## 確定シフト")
        fixed_shift_data = pd.read_excel(input_file, "fixed_shift_test", dtype=str)
        st.table(fixed_shift_data)